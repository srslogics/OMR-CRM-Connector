...
const record={action:'save',id:crypto.randomUUID(),name:'Test Student',school:'Test School',fatherPhone:'8888888888',motherPhone:'',className:'10'};
...
const visible=await request('/api/workspace',null,owner.cookie);assert.equal(visible.data.records.length,1);assert.equal(visible.data.records[0].name,'Test Student');
assert.equal((await request('/api/workspace',{...record,name:'Owner correction'},owner.cookie)).status,200);
const csv=await request('/api/workspace?export=csv',null,owner.cookie);assert.equal(csv.status,200);assert.ok(csv.data.includes('Owner correction'));
